;; 1. Based on: 3100
;; 2. Description: PQ new; hill ICU+d, Hosp BA2, Hosp leveln(incl ID 12+17), Hosp+ICU+Vent BA45
;; x1. Author: dings
...
DSMBA2 = (TIME-StarttagBA2)*MPAST(37)
...
AnteilBA2 = 0.99/(1+((1-initBA2)/initBA2)*EXP(-kBA2*DSMBA2))*MPAST(37)
...
MTIME(52) = THETA(55)
F_Hosp_all = 1*THETA(39)**MPAST(52)
...
IF(ID.EQ.12) F_Hosp_all = THETA(40)**(MPAST(6)-MPAST(52))*THETA(39)**MPAST(52)
IF(ID.EQ.17) F_Hosp_all = THETA(41)**(MPAST(6)-MPAST(52))*THETA(39)**MPAST(52)

MTIME(46) = 600
hill_2022 = THETA(50)
FHosp_2022 = 1-THETA(49)*(MPAST(46)*(TIME-600))**hill_2022/((MPAST(46)*(TIME-600))**hill_2022+(THETA(46)-600)**hill_2022)
hospfactor = F_Hosp_all*FHosp_2022*(1-FKH*TIME**hill2/(THETA(4)**hill2+TIME**hill2))*EXP(-FKHnTEST*AnzahlTestungen/1000000)*(1+(AnteilAlpha-AnteilDelta*0.92/0.99)*THETA(20))*(1+AnteilDelta*THETA(28))*(1+AnteilOmicron*THETA(34))*(1+AnteilBA2*THETA(54))*(1+AnteilBA4_5*THETA(51))
...
F_VENT =  (1+AnteilOmicron*THETA(44))*(1+AnteilBA4_5*THETA(56))
...
(0, 0.87,2) FIX ;39 F_Hosp
(0, 1.07,2) FIX ;40 F_Hosp
(0, 0.894,2) FIX ;41 F_Hosp
(-0.9, -0.493,0) FIX ;42 F_ICU_Omicron
(0) FIX ;43 F_LD_Omicron
(-0.9, -0.302,0) FIX ;44 F_Vent_Omicron
(0, 0.309,1) FIX ; 45 F_LD_ICU_Om
(600, 786,830) FIX ;46 MT ICU
(0, 0.738,1) ;47 F_ICU2022
(0, 0.599,1) ;48 F_Death2022
(0) FIX ;49 F_Hosp2022
(0, 8.12,50) ;50 hill_2022
(-1, 0.3,10) ;51 F_Hosp_BA45 Zugewinn Hospitalisierungsrate durch BA4/5 
(-1, 0.434,10) ;52 F_ICU_BA45 Zugewinn ICUrate durch BA4/5 
(-1, 0,10) FIX ;53 F_Death_BA45 Zugewinn Sterberate durch BA4/5 
(-1, -0.327,0) ;54 F_Hosp_BA2 Zugewinn Hospitalisierungsrate durch BA2 
(790, 823) FIX ;55 MT Hosp
(-1, -0.326,10) ;56 F_Vent_BA45
...
$TABLE ID TIME DV OBS CMT EVID MDV IPRED Rt R0FREE R0FREE_OLD hosp_TV hospfactor hosp FRACTHOSP FRACTICU KHd ICUd BEATd OUTd KHd80 death ONEHEADER NOPRINT FILE=sdtab3100R
