;; 1. Based on: 1100
;; 2. Description: hosp+d+ICU+F_LD_ICU+Vent+LD_KH_L(O); cp Inf+Hosp(BaWue); F RR_Hosp+ICU+dout; F_Hosp(BL); hill ICU+d+hosp; PQ est
;; x1. Author: user
...
;Parameter VOC Omicron BA.5
StarttagBA5 = 347+501
kBA5= 0.1083
initBA5= 0.002
FaktorBA5 = 1.97*1.62*1.5*1.57   ;Factor BA5 vs WT
...
hospfactor = F_Hosp_all*FHosp_2022*(1-FKH*TIME**hill2/(THETA(4)**hill2+TIME**hill2))*EXP(-FKHnTEST*AnzahlTestungen/1000000)*(1+(AnteilAlpha-AnteilDelta*0.92/0.99)*THETA(20))*(1+AnteilDelta*THETA(28))*(1+AnteilOmicron*THETA(34))
...
FICU= CPs_ICU*(1+(AnteilAlpha-AnteilDelta*0.92/0.99)*THETA(21))*(1+AnteilDelta*THETA(30))*(1+AnteilOmicron*THETA(42))
...
KHd = (0.0125*KHu60_M+0.0125*KHu60_W+0.147*KHue60_M+0.101*KHue60_W+0.414*KHue80_M+0.334*KHue80_W)*(1+(AnteilAlpha-AnteilDelta*0.92/0.99)*THETA(19))*(1+AnteilDelta*THETA(19))*PQdeath*(1+AnteilOmicron*THETA(35))*FDeath_2022
KHd80 = 0.414*PQdeath*(1+(AnteilAlpha-AnteilDelta*0.92/0.99)*THETA(19))*(1+AnteilDelta*THETA(19))*(1+AnteilOmicron*THETA(35))*FDeath_2022
ICUd = (0.0453*ICUue35+0.193*ICUue60+0.477*ICUue80) *(1+(AnteilAlpha-AnteilDelta*0.92/0.99)*THETA(19))*(1+AnteilDelta*THETA(19))*PQdeath*(1+AnteilOmicron*THETA(35))*FDeath_2022
BEATd = (0.25*Bue05+0.18*Bue15+0.372*Bue35+0.653*Bue60+0.841*Bue80) *(1+(AnteilAlpha-AnteilDelta*0.92/0.99)*THETA(19))*(1+AnteilDelta*THETA(19))*PQdeathICU*(1+AnteilOmicron*THETA(35))*FDeath_2022
...
OUTd = (THETA(16)+FDOUT-FDOUT2)*PQdeath*(1+(AnteilAlpha-AnteilDelta*0.92/0.99)*THETA(19))*(1+AnteilDelta*THETA(19))*(1+AnteilOmicron*THETA(35))*FDeath_2022
...
DaysSinceBA5 = (T-StarttagBA5)*MPAST(37)
BA5 = 0.99/(1+((1-initBA5)/initBA5)*EXP(-kBA5*DaysSinceBA5))*MPAST(37)

R0FREE = (R0FREE_OLD*(1-(Alpha-Delta*0.92/0.99)-Delta)+R0FREE_OLD*1.35*(Alpha-Delta*0.92/0.99)) + R0FREE_OLD*FaktorDelta*(Delta-Omicron) + R0FREE_OLD*FaktorOmicron*Omicron*(1-BA2) + R0FREE_OLD*FaktorBA2*Omicron*(BA2-BA5) + R0FREE_OLD*FaktorBA5*Omicron*BA5
beta = R0FREE*gamma
...
DaysSinceBA5_Err = TIME-StarttagBA5
BA5_Err = 0.99/(1+((1-initBA5)/initBA5)*EXP(-kBA5*DaysSinceBA5_Err))
R0 = R0FREE_OLD*(1-(Alpha_Err-Delta_Err*0.92/0.99)-Delta_Err)+R0FREE_OLD*1.35*(Alpha_Err-Delta_Err*0.92/0.99) + R0FREE_OLD*FaktorDelta*(Delta_Err-Omicron_Err) + R0FREE_OLD*FaktorOmicron*Omicron_Err*(1-BA2_Err) + R0FREE_OLD*FaktorBA2*Omicron_Err*(BA2_Err-BA5_Err) + R0FREE_OLD*FaktorBA5*Omicron_Err*BA5_Err
SUSCEPTIBLES = A(1)
...
(600, 794,830) ;46 MT ICU
(0, 0.801,1) ;47 F_ICU2022
(0, 0.559,1) ;48 F_Death2022
(0, 0.316,1) ;49 F_Hosp2022
(0, 7.92,50) ;50 hill_2022
...
$TABLE ID TIME DV OBS CMT EVID MDV IPRED Rt R0FREE R0FREE_OLD hosp_TV hospfactor hosp FRACTHOSP FRACTICU KHd ICUd BEATd OUTd KHd80 death ONEHEADER NOPRINT FILE=sdtab1100R
