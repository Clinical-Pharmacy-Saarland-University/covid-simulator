;; 1. Based on: 2102
;; 2. Description: Hosp BA2, Hosp+Vent BA45, cp hosp+ICU, est RR_Hosp_O+Hosp+hill(2), cp Hosp+ICU+death, -dout(CPhosp), GER first
;; x1. Author: dings
...
$DATA ..\DATASET\COVID_ds_256_Impf_VOC_Betten_V3_R0_2006_44.csv IGNORE=@ IGNORE(ID.EQ.17) IGNORE(DOSING2.EQ.1) IGNORE(DIVIFLAG.EQ.2) IGNORE(CMT.EQ.8) IGNORE(CMT.EQ.11) IGNORE(CMT.EQ.12) IGNORE(CMT.EQ.9) IGNORE(FLAGZEROOBS.EQ.1) IGNORE(FLAGRKI.EQ.1) IGNORE(PREDIC.EQ.1)
...
kXBB= 0.077
initXBB= 0.002
FaktorXBB = 1.97*1.62*1.5*1.77*1.39  ;Factor XBB vs WT
...
AnteilBA2 = 0.99/(1+((1-initBA2)/initBA2)*EXP(-kBA2*DSMBA2))*MPAST(10)
AnteilBA4_5 = 0.99/(1+((1-initBA4_5)/initBA4_5)*EXP(-kBA4_5*DSMBA4_5))*MPAST(10)
...
hospfactor = F_Hosp_all*FHosp_2022*(1-FKH*TIME**hill2/(280**hill2+TIME**hill2))*NTests*(1+(AnteilAlpha-AnteilDelta*0.92/0.99)*0.124)*(1+AnteilDelta*0.284)*(1+AnteilOmicron*(-0.589))*(1+AnteilBA2*THETA(14))*(1+AnteilBA4_5*THETA(11))
...
OUTd = (0+FDOUT-FDOUT2)*PQdeath*(1+(AnteilAlpha-AnteilDelta*0.92/0.99)*0)*(1+AnteilDelta*0)*(1+AnteilOmicron*(-0.577))*(1+AnteilBA4_5*THETA(13))*FDeath_2022*FDOUT3
...
R0FREE = R0FREE_OLD*((1-(Alpha-Delta*0.92/0.99)-Delta)+1.35*(Alpha-Delta*0.92/0.99) + FaktorDelta*(Delta-Omicron) + FaktorOmicron*Omicron*(1-BA2) + FaktorBA2*Omicron*(BA2-BA4_5) + FaktorBA4_5*Omicron*(BA4_5-XBB) + FaktorXBB*XBB)
beta = R0FREE*gamma
...
DADT(31) = (Omicron-XBB)*gamma*A(2) - beta*XBB*A(31)/NUMBER*A(2)*(Ungeimpft+ZweitGesamt*FractionInfectious+BoosterGesamt*FractionInfectious_Booster)
...
Omicron_Err = 0.99/(1+((1-initOmicron)/initOmicron)*EXP(-kOmicron*DaysSinceOmicron_Err))
IF(TIME.LT.StarttagOmicron) Omicron_Err = 0
;BA.2
DaysSinceBA2_Err = TIME-StarttagBA2
IF(TIME.LT.StarttagBA2) DaysSinceBA2_Err = 0
BA2_Err = 0.99/(1+((1-initBA2)/initBA2)*EXP(-kBA2*DaysSinceBA2_Err))
...
;R0FREE
R0 = R0FREE_OLD*((1-(Alpha_Err-Delta_Err*0.92/0.99)-Delta_Err)+1.35*(Alpha_Err-Delta_Err*0.92/0.99) + FaktorDelta*(Delta_Err-Omicron_Err) + FaktorOmicron*Omicron_Err*(1-BA2_Err) + FaktorBA2*Omicron_Err*(BA2_Err-BA4_5_Err) + FaktorBA4_5*Omicron_Err*(BA4_5_Err-XBB_Err)+FaktorXBB*XBB_Err)
SUSCEPTIBLES = A(1)
...
(-0.9, -0.0777,0) FIX ;2 F_ICU_Omicron
(-0.9, -0.349,0) FIX ;3 F_Vent_Omicron
(0) FIX ;4 F_LD_ICU_Om
(0, 0.399) FIX ;5 FKHTests1
(600, 769,830) FIX ;6 MT ICU
(0, 0.714,1) FIX ;7 F_ICU2022
(0, 0.653,1) FIX ;8 F_Death2022
(0) FIX ;9 F_Hosp2022
(0, 17.7,50) FIX ;10 hill_2022
(-1, 0.673,10) FIX ;11 F_Hosp_BA45 Zugewinn Hospitalisierungsrate durch BA4/5 
(0) FIX ;12 F_ICU_BA45 Zugewinn ICUrate durch BA4/5 
(0) FIX ;13 F_Death_BA45 Zugewinn Sterberate durch BA4/5 
(-1, -0.531,0) FIX ;14 F_Hosp_BA2 Zugewinn Hospitalisierungsrate durch BA2 
(790, 828) FIX ;15 MT Hosp
(-1, -0.222,10) FIX ;16 F_Vent_BA45
...
(0.4, 1.5) FIX ;26 PQ2
(0, 0.46,1) ;27 RR_Hosp_O Omicron
(1010, 1070,1120) ;28 MTIME
(1, 6.73,50) ;29 hill_2022_2
(-1, 0.601,10) ;30 death
(0, 0.785,1) ;31 fdout
...
$TABLE ID TIME DV OBS CMT EVID MDV IPRED Rt R0FREE R0FREE_OLD hosp_TV hospfactor hosp FRACTHOSP FRACTICU KHd ICUd BEATd OUTd KHd80 death ONEHEADER NOPRINT FILE=sdtab2102R
